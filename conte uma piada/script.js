...
// desabilida/habilita Button
function toggleButton() {
  button.disabled = !button.disabled;
}

// VoiceRSS Speech Function
function contarPiada(joke){
  const jokeString = joke.trim().replace(/ /g, '%20');
  // VoiceRSS Speech Parameters
  VoiceRSS.speech({
    key: 'b12104e9760840159abe9128a2a77745',
    src: jokeString,
    hl: 'en-us',
    v: 'Linda',
    r: 0, 
...
async function pegarPiada() {
  ...
  try {
    ...
    // ve se é de 1 ou 2 partes
    if (data.setup) {
      joke = `${data.setup} ... ${data.delivery}`;
    } else {
      joke = data.joke;
    }
    // passa a piada para o VoiceRSS API
    contarPiada(joke);
    // desabilita botao Button
    toggleButton();
  } catch (error) {
    // Catch Error Here
  }
